...
import telethon.crypto.srp as srp_module
from telethon.crypto import srp as srp_module
...
def manual_compute_check(password, algo):
    """
    Ручная реализация SRP. Работает с любыми паролями.
    """
    # !!! ИСПРАВЛЕНИЕ 1: ПРЕВРАЩАЕМ ПАРОЛЬ В СТРОКУ !!!
    ...
    return tl_types.InputCheckPasswordSRP(
        srp_id=algo.srp_id,
...
async def compute_2fa_hash(client, password, pwd_info):
    """
    Универсальный расчет 2FA, устойчивый к ошибкам импорта SRP.
    """
    if not password:
        return None
    ...
    try:
        # Пытаемся достать SRP через глобальный модуль, который мы импортировали в начале
        if 'srp_module' in globals() and hasattr(srp_module, 'SRP'):
            return srp_module.SRP.compute_check(
                pwd_info.current_algo,
                pwd_info.srp_B,
                pwd_info.srp_id,
                password
            )
        
        # Если в глобальных нет, пробуем достать напрямую из клиента (внутренний метод)
        return await client._compute_check_password_srp(password, pwd_info)

    except Exception as e:
        # Последний шанс: импорт "на лету" внутри функции
        try:
            import telethon.crypto.srp as srp_internal
            return srp_internal.SRP.compute_check(
                pwd_info.current_algo,
                pwd_info.srp_B,
                pwd_info.srp_id,
                password
            ...
            print(f"!!! КРИТИЧЕСКАЯ ОШИБКА SRP: {final_err}")
            return None
...
def start_process(mode="smart"):
    ...
    try:
        ...
        # 1. ЛОГИКА ВЫБОРА МЕЙКЕРА (из клика или из галочек)
        if not current_maker_phone and selected_maker_phones:
            current_maker_phone = list(selected_maker_phones)[0]
            log_msg("INFO", f"Выбран мейкер из галочек: {current_maker_phone}")

        if not current_maker_phone:
            messagebox.showwarning("!", "Выберите мейкера в таблице или галочкой!")
            return
        ...
        for idx, s in enumerate(sessions_data):
            ...
            # Ищем мейкера
            clean_m = str(current_maker_phone).replace(" ", "").replace("-", "").replace("(", "").replace(")", "")
            if s_phone == clean_m:
                maker_indices.append(idx)
            
            # Ищем директора (GUEST)
            if current_director_phone:
                clean_d = str(current_director_phone).replace(" ", "").replace("-", "").replace("(", "").replace(")", "")
                if s_phone == clean_d:
                    guest_index = idx
...
class TapokWindow(Toplevel):
    ...
    async def single_maker_worker(self, session_data, anchors_list, count, password):
        ...
        try:
            ...
            pwd_info = None
            if password:
                try: pwd_info = await client(functions.account.GetPasswordRequest())
                except: pass
            ...
            for i in range(1, count + 1):
                ...
                try:
                    ...
                    # 2. ПРАВА МЕЙКЕРА (Определяем переменную)
                    full_rights = ChatAdminRights(
                        ...
                        manage_call=True,
                        other=True,
                        manage_topics=True,  # Темы
                        post_stories=True,   # История 1
                        edit_stories=True,   # История 2
                        delete_stories=True  # История 3
                    )
                    
                    # 2.1 ПРИМЕНЯЕМ ПРАВА (Этой строки у вас не хватало!)
                    try: 
                        await client(EditAdminRequest(chat, 'me', full_rights, "F"))
                    except Exception as e:
                        # self.log(f"Warning rights: {e}") # Можно раскомментить для отладки
                        pass
                    ...
                    if password and pwd_info:
                        try:
                            # А) ДАЕМ АДМИНКУ ЯКОРЮ (ОБЯЗАТЕЛЬНО)
                            await client(EditAdminRequest(chat, target, full_rights, "Owner"))
                            
                            # !!! Б) ПАУЗА ЧТОБЫ ТГ ОСОЗНАЛ, ЧТО ОН АДМИН !!!
                            await asyncio.sleep(2) 
                            
                            # В) СЧИТАЕМ ХЭШ
                            srp_hash = await compute_2fa_hash(client, password, pwd_info)
                            ...
                            if srp_hash:
                                ...
                                self.log(f"[{short_ph}] ❌ Ошибка: Хэш не рассчитан.", "ERROR")
