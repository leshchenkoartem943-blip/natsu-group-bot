...
class SmartAssistant:
    ...
    def setup(self, key):
        ...
        try:
            ...
            self.model = genai.GenerativeModel('gemini-1.5-flash') # Быстрая и дешевая модель
        except:
            pass

    def analyze_error(self, context, error_text):
        """Отправляет ошибку в ИИ и получает совет"""
        if not self.model: return None
        
        prompt = (
            f"Ты — технический помощник для Python-программы автоматизации Telegram (Telethon). "
            f"Произошла ошибка во время действия: '{context}'. "
            f"Текст ошибки: '{error_text}'. "
            f"Кратко (в 1-2 предложениях) объясни причину и напиши, что пользователю сделать, чтобы исправить. "
            f"Не пиши код, пиши совет для пользователя."
        )
        try:
...
def open_settings():
    # Увеличили высоту окна, чтобы влезли настройки ИИ
    win = Toplevel(root); win.title("Настройки"); win.geometry("450x680")
    ...
    # === 1. БЛОК ИИ (НОВОЕ) ===
    lf_ai = ttk.LabelFrame(main_frame, text=" 🤖 ИИ Ассистент (Google Gemini) ", padding=10)
    lf_ai.pack(fill="x", pady=5)
    
    ttk.Label(lf_ai, text="API Key (Google AI Studio):").pack(anchor="w")
    e_ai_key = ttk.Entry(lf_ai, width=40)
    e_ai_key.pack(fill="x", pady=5)
    # Загружаем сохраненный ключ, если есть
    e_ai_key.insert(0, cfg.get("ai_api_key", ""))
    
    ttk.Label(lf_ai, text="ИИ будет анализировать ошибки и давать советы.", font=("Arial", 8), foreground="gray").pack(anchor="w")
    ...
    def save():
        ...
        # Обновляем помощника, чтобы он заработал сразу без перезапуска
...
def open_accounts():
    ...
    def clear_contacts_action():
        ...
        # Безопасная модальность (если make_modal нет)
        try:
        ...
        def runner():
            ...
            async def work():
                try:
                    ...
                    # ИСПРАВЛЕНИЕ: Берем объекты User целиком, а не ID
                    me = await client.get_me()
                    users_to_del = [u for u in contacts.users if u.id != me.id]
                    ...
                    for i in range(0, total, chunk_size):
                        ...
                        try:
                            # Передаем объекты пользователей, Telethon сам разберется с InputUser
                    ...
                    p_win.after(0, lambda: messagebox.showerror("Ошибка", str(e)))
                    p_win.after(0, p_win.destroy)
                finally:
                    if client.is_connected(): await client.disconnect()
    ...
    ttk.Button(btn_frame, text="🧹 Очистить контакты", command=clear_contacts_action).pack(side="left", fill="x", expand=True, padx=5)
