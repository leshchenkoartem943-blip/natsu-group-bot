...
async def auto_setup_profile(client, first_name, last_name="", is_director=False):
    """
    Только удаляет аватарки и настраивает приватность (если директор).
    Имя, описание (Bio) и юзернейм НЕ трогает.
    """
    try:
        log_msg("INFO", "⚙️ Настройка профиля (только фото и приватность)...")
        
        # 1. Смена Имени и удаление Описания -> ОТКЛЮЧЕНО
        # await client(UpdateProfileRequest(first_name=first_name, last_name=last_name, about=""))

        # 2. Удаление Юзернейма -> ОТКЛЮЧЕНО
        # try:
        #     await client(UpdateUsernameRequest(username=""))
        # except Exception: pass
        
        # 3. Удаление ВСЕХ аватарок -> ОСТАВЛЕНО
        try:
            ...
            log_msg("WARN", f"   ⚠️ Не удалось удалить фото: {e}")

        # 4. Приватность (ТОЛЬКО ДЛЯ ДИРЕКТОРА) -> ОСТАВЛЕНО (Важно для работы)
        if is_director:
            try:
                ...
                log_msg("WARN", f"   ⚠️ Ошибка приватности: {e}")
