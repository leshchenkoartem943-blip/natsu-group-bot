...
async def process_smart_target_file(maker_client, guest_client, file_path, guest_session_dict=None):
    try:
        ...
        # === ПРЕДВАРИТЕЛЬНОЕ ОБНОВЛЕНИЕ ИМЕН (Если авторизованы) ===
        # (Оставляем тут try-pass, так как полноценная проверка будет ниже по кнопке "НЕТ")
        ...
        try:
            ...
            with open(full_path, "w", encoding="utf-8") as f:
                ...
                for m in selected_matches:
                    ...
                    fio_line = m['target_fio']
                    if m.get('target_dob'):
                        fio_line += f" {m['target_dob']}"
                    
                    f.write(f"👤 КЛИЕНТ:  {fio_line}\n")
        ...
        def ask_go_dialog():
            ans = messagebox.askyesno(
                ...
                "ДА -> Запустить СРАЗУ.\n"
                "НЕТ -> Обновить профили, АВТОРИЗОВАТЬСЯ и запустить."
            )
        ...
        if not is_immediate_start:
            log_msg("WAIT", "🔄 Проверка авторизации и обновление профилей...")
            
            # === ВАЖНО: АВТОРИЗАЦИЯ ГОСТЯ ===
            if guest_client and guest_session_dict:
                try:
                    # 1. Если клиент отключен, подключаем
                    if not guest_client.is_connected():
                         await guest_client.connect()
                    
                    # 2. ПРОВЕРКА: Если не авторизован - ВХОДИМ
                    if not await guest_client.is_user_authorized():
                        g_phone = guest_session_dict['phone']
                        log_msg("WARN", f"🔐 ГОСТЬ {g_phone} не в системе! Запрашиваю код...")
                        
                        await guest_client.send_code_request(g_phone)
                        code = await asyncio.get_running_loop().run_in_executor(None, ask_code_gui, g_phone, False)
                        if code:
                            try:
                                await guest_client.sign_in(g_phone, code)
                            except SessionPasswordNeededError:
                                pwd = await asyncio.get_running_loop().run_in_executor(None, ask_code_gui, g_phone, True)
                                await guest_client.sign_in(password=pwd)
                            
                            log_msg("SUCCESS", f"✅ Гость {g_phone} успешно вошел!")
                        else:
                             log_msg("ERROR", f"❌ Код не введен, Гость пропущен.")

                    # 3. Теперь, когда вход гарантирован, обновляем профиль
                    me_g = await guest_client.get_me() # Теперь это не упадет
                    
                    # Обновляем имя (если прописано в файле)
                    if parsed_data['director_name']:
                        try:
                            d_name = parsed_data['director_name']
                            d_parts = d_name.split()
                            if len(d_parts) >= 2:
                                ...
                                new_last = ""
                            await guest_client(functions.account.UpdateProfileRequest(first_name=new_first, last_name=new_last))
                            me_g = await guest_client.get_me() # Обновляем объект
                        except: pass
                    ...
                    log_msg("INFO", f"   👤 Гость готов: {new_name}")

                except Exception as e:
                     log_msg("WARN", f"⚠️ Ошибка настройки Гостя: {e}")

            # Обновление Мейкера (он обычно уже в сети, но на всякий случай)
            try:
                me_m = await maker_client.get_me()
                nm = f"{me_m.first_name} {me_m.last_name or ''}".strip()
                log_msg("INFO", f"   🤖 Мейкер готов: {nm}")
            except: pass
                
            log_msg("SUCCESS", "✅ Все готово к запуску.")
        else:
            log_msg("INFO", "🚀 Мгновенный запуск...")
...
async def worker_task(session, names, delays, target_username, smart_file_path=None, guest_session=None):
    ...
    try:
        ...
        # === ! ПРИМЕНЯЕМ НОВЫЕ НАСТРОЙКИ ПРИВАТНОСТИ К МЕЙКЕРУ ! ===
        await setup_privacy_security(client, "Maker")
        ...
        if smart_file_path:
            ...
            if guest_session:
                try:
                    # Подключаем гостя только чтобы пробить/настроить
            ...
            if not smart_users:
                log_msg("WARN", "⚠️ Кандидаты отменены или не выбраны.")
                return []
        ...
        # === 1. ПОДГОТОВКА ГОСТЯ (ДОБАВЛЯЕМ В КОНТАКТЫ НАВСЕГДА) ===
        target_user_entity = None
        
        if guest_session:
            guest_uname = guest_session.get('username')
            guest_phone = guest_session['phone']
            
            try:
                # Попытка 1: По юзернейму
                if guest_uname:
                    target_user_entity = await client.get_input_entity(guest_uname)
                
                # Попытка 2: Импорт контактов (ЭТО ОСНОВНОЙ МЕТОД ДЛЯ ВЗАИМНОСТИ)
                # Даже если нашли по юзернейму, лучше добавить в контакты, чтобы стать "Mutual"
                log_msg("WAIT", "   🔍 Добавляем Гостя в контакты (для 'Mutual' статуса)...")
                contact_to_import = types.InputPhoneContact(
                    client_id=random.randint(10000, 99999),
                    phone=guest_phone,
                    first_name="Guest",
                    last_name="Partner"
                )
                res = await client(functions.contacts.ImportContactsRequest(contacts=[contact_to_import]))
                
                if res.users:
                    target_user_entity = res.users[0]
                    log_msg("SUCCESS", "      ✅ Гость добавлен в контакты (не удаляем).")
                else:
                    # Если уже в контактах, пробуем найти
                    if not target_user_entity:
                         cts = await client(functions.contacts.GetContactsRequest(hash=0))
                         clean_gp = guest_phone.replace(" ", "").replace("+", "").strip()
                         for u in cts.users:
                             if u.phone and clean_gp in u.phone:
                                 target_user_entity = u
                                 break
            except Exception as e:
                log_msg("WARN", f"   ⚠️ Ошибка работы с контактом Гостя: {e}")
        
        # === 2. ПОЛУЧЕНИЕ КОНТАКТОВ ДЛЯ ИНВАЙТА ===
        contact_users = []
        ...
        for i, name in enumerate(names):
            if stop_flag.is_set(): break
            log_msg("INFO", f"🛠 ({i+1}/{total_groups}) Создание: {name}")
        ...
        if delayed_invites and not stop_flag.is_set():
            log_msg("INFO", f"📥 {phone}: Инвайт отложенных...")
...
async def guest_execution_final(session, all_maker_ids, target_group_ids, greeting_text):
    ...
    try:
        ...
        # === ! ПРИМЕНЯЕМ НАСТРОЙКИ ПРИВАТНОСТИ К ГОСТЮ ! ===
        await setup_privacy_security(client, "Guest")
...
def run_thread(main_sessions, guest_session, names, delays, target_username_manual, greeting_text, need_greet, smart_file_path=None):
    ...
    log_msg("INFO", f"🎯 Цель: @{target_username}")
    if smart_file_path:
        log_msg("INFO", f"📂 Файл: {os.path.basename(smart_file_path)}")
    ...
    try:
        if maker_tasks:
            ...
            if guest_session and not stop_flag.is_set() and all_created_groups and need_greet:
                log_msg("INFO", "\n=== ЗАПУСК ГОСТЯ ===")
                # Пауза всего 1 секунда (нужна, чтобы сервер Телеграма успел "понять", что группы созданы)
                log_msg("WAIT", "⏳ Сек..")
                time.sleep(1)
                
                # ВАЖНО: Вызываем БЕЗ 'names', так как поиск строго по ID
...
def start_process(mode="smart"):
    try:
        # Теперь всегда работаем только через Smart (выбор файла)
        ...
        # УБРАЛИ чтение ent_name, ent_count, ent_user
        # Вместо этого передаем заглушки или пустые значения, 
        # так как в Smart режиме они пересчитываются из файла.
        
        base_name = "SmartMode" # Не используется, так как имя берется из файла
        dummy_names = []         # Не используется
        manual_username = ""     # Директор берется из выбранного guest_session

        # Блокируем кнопки
        smart_btn.config(state='disabled')
        
        threading.Thread(
            target=run_thread,
            # Передаем smart_path последним аргументом
            args=(main_sessions, guest_session, dummy_names, delays, manual_username, greeting_text, need_greet, smart_path),
            daemon=True
        ).start()
...
def create_dashboard_tab(parent):
    # УБРАЛИ top_frame (Настройки группы, количество, юзернейм)
    
    # 1. ЦЕНТР - СПИСКИ (Теперь они занимают верхнюю часть)
    ...
    def on_entry_in(e):
        if ent_guest_search.get() == "поиск":
            ...
            ent_guest_search.configure(foreground="white")
    ...
    global smart_btn
    # УБРАЛИ start_btn (Обычный)
    
    # Кнопка Смарт теперь главная и зеленая
    smart_btn = ttk.Button(btn_area, text="📂 СТАРТ ПО БАЗЕ (Smart)", command=lambda: start_process("smart"), style="Green.TButton")
    smart_btn.pack(side="left", fill="x", expand=True, padx=(0, 5), ipady=8) # ipady побольше для массивности
...
class SidebarApp:
    def __init__(self, root):
        ...
        self.frames["Databases"] = ttk.Frame(self.content_area) # <-- ДОБАВЛЕНО
        ...
        create_databases_tab(self.frames["Databases"]) # <-- ВЫЗОВ НОВОЙ ФУНКЦИИ
        ...
        self._add_menu_btn("📂 Пробитые Базы", "Databases") # <-- НОВАЯ КНОПКА
